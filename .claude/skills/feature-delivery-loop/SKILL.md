...
9. **`.gitattributes` 每次同步都冲突，且有两种形态**。前两次是两侧**各自追加**（保留两边即可）；
   #25 是两侧**改到同一区域**——上游删掉了已废弃的块并改写了注释，而那块正是我们逐字导入的。
   这时**不能简单两边保留**（会留下上游已删除的规则），要看清上游当前的实际内容再对齐。
   **通用判据**：解析完之后跑
   `git diff upstream/main -- .gitattributes`，输出应当**只剩 fork 自己要加的那几行**
   （本仓库是 9 行 Trellis journal 规则）。多出来的任何一行都是偏离，会在下次同步变成新冲突。
   解析完再 `git check-attr` 验证 fork 自己的规则确实生效（`merge: union`）。
10. **同步不一定要出新包**：若 `mobile-harmony/` 零改动，用
